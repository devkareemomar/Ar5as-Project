...
if (! function_exists('filter_products')) {
    function filter_products($products) {
        ...
        if(BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1){
            return $products->where([['admin_approval', '1'],['published', '1']])->orderBy('created_at', 'desc')->where(function($p) use ($verified_sellers){
            ...
            return $products->where([['admin_approval', '1'],['published', '1']])->where('added_by', 'admin');
        }
    }
}
...
if (! function_exists('get_cached_products')) {
    function get_cached_products($category_id = null) {
        $products = \App\Product::where([['admin_approval', 1],['published', 1]]);
        $verified_sellers = verified_sellers_id();
        if(BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1){
...
//Shows Price on page based on low to high
if (! function_exists('defulat_price')) {
    function defulat_price($id)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
...
function shippingCostByCityId($id){
    $cost = \App\City::where('id', $id)->select('cost')->first();
