                                                ...
                                                <span class="flex-grow-1 pl-3 text-left">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span class="fw-600 ml-2">
                                                            @php
                                                                $city = App\City::where('id',$address->city )->select('name')->first()
                                                            @endphp
                                                            {{ $city->name }}
                                                        </span>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <span class="fw-600 ml-2">
                                                              @php
                                                                $gov = App\Governorate::where('id',$address->country )->select('name')->first()
                                                            @endphp
                                                            {{ $gov->name }}
                                                        </span>
                                                    </div>
                ...
                <div class="modal-body">
                    <div class="p-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-10">
                                <select id="govs" class="form-control mb-3 aiz-selectpicker" data-live-search="true" name="gov_id" required>
                                     <option  value="" disabled selected> اختر المحافظة</option>
                                    @foreach (\App\Governorate::where('status', 1)->get() as $key => $country)
                        ...
                        @if (\App\BusinessSetting::where('type', 'shipping_type')->first()->value == 'area_wise_shipping')
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    <select id="cites" class="form-control mb-3 aiz-selectpicker" data-live-search="true" name="city" required>
                                        {{--  @foreach (\App\City::get() as $key => $city)
                                            <option value="{{ $city->name }}">{{ $city->getTranslation('name') }}</option>
                                        @endforeach  --}}
                                    </select>
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
        $("#govs").change(function(){
            $.ajax({
                url: "{{ route('citeybygovs') }}?gov_id=" + $(this).val(),
                method: 'GET',
                success: function(data) {
                    $('#cites').html(data.html);
                }
            });
        });
    </script>
