...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header row gutters-5">
              ...
              @if ($order->order_status == 'picked' || $order->order_status == 'on_delivery'  || $order->order_status == 'customer_not_found')
                  ...
                  <select class="form-control aiz-selectpicker"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                      ...
                      <option value="customer_not_found" @if ($delivery_status == 'customer_not_found') selected @endif>{{translate('Customer Not Found')}}</option>
              ...
              <div class="col-md-3 ml-auto">
                  <label for=update_shipping_status"">{{translate('Shipping By')}}</label>
                  <select class="form-control aiz-selectpicker" data-live-search="true" id="update_shipping_status">
                    @php 
                            $companies = App\ShippingCompany::get();
                    @endphp
                    
                    @foreach ( $companies as $company )
                      <option value="{{ $company->id }}" @if ($order->shipping_by == $company->id) selected @endif>{{ $company->name }}</option>
                        ...
                        <td>{{ $orderDetail->variation }}</td>
                         ...
                         <td>
                            ...
                            <td>
                              ...
                              @if($orderDetail->is_deleted == 0)

                                @if ($orderDetail->at_stock == 0)
                                  <label class="aiz-switch aiz-switch-success mb-0">
                                    <input onchange="update_status(this)" value="{{ $orderDetail->id }}" type="checkbox" <?php if($orderDetail->at_stock == 1) echo "checked";?> >
                                    <span class="slider round"></span>
                                  </label>
                                @elseif ($orderDetail->at_stock == 1)
                                  <i class="las la-check-circle" style="color:green; font-size: 32px"></i>
                                @endif
                              @elseif($orderDetail->is_deleted == 1)
                              {{ translate('Item Removed From Order') }}
                              @endif
                            </td>
    		...
    		<div class="clearfix float-right">
    			<table class="table">
                    ...
                    <tr>
        				...
        				<td>
        					{{ single_price($order->orderDetails->where('is_deleted' , '=' , 0)->sum('shipping_cost')) }}
        				</td>
        			</tr>
                    <tr>
        				<td>
...
@section('script')
    <script type="text/javascript">
        ...
        $('#update_shipping_status').on('change', function(){
            var order_id = {{ $order->id }};
