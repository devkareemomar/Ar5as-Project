...
<head>
    ...
    <title>{{  translate('Ar5as Invoicde') }}</title>
	...
	<style media="all">
	    ...
	    @elseif($currency_code == 'EGP88')
			@font-face {
	            font-family: 'Cairo';
	            src: url("{{ static_asset('assets/fonts/Tahoma-Regular-font.ttf') }}") format("truetype");
	            font-weight: normal;
	            font-style: normal;
	        }
	        *{
	...
	<div>
		...
		<div style="background: #eceff4;padding: 1rem;">
			...
			<table>
				...
				<tr>
					...
					<td class="text-right small"><span class="gry-color small">{{  translate('Payment Type') }}:</span> <span class=" strong">{{ ucfirst(str_replace('_', ' ', $order->payment_type)) }}</span></td>
				</tr>
	    ...
	    <div style="padding: 1rem;">
			<table class="padding text-left small border-bottom">
				...
				<tbody class="strong">
	                @foreach ($order->orderDetails->where('is_deleted', '=', 0) as $key => $orderDetail)
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			            ...
			            <td class="currency">{{ single_price($order->orderDetails->where('is_deleted' , '=' , 0)->sum('price')) }}</td>
			        </tr>
			        <tr>
			            <th class="gry-color text-left">{{ translate('Shipping Cost') }}</th>
			            <td class="currency">{{ single_price($order->orderDetails->where('is_deleted' , '=' , 0)->sum('shipping_cost')) }}</td>
			        </tr>
			        <tr class="border-bottom">
			            <th class="gry-color text-left">{{ translate('Total Tax') }}</th>
			            <td class="currency">{{ single_price($order->orderDetails->where('is_deleted' , '=' , 0)->sum('tax')) }}</td>
			        </tr>
                    <tr class="border-bottom">
