                ...
                <div class="row">
                    <div class="col"> 
                        <div class="aiz-carousel gutters-10" data-items="6" data-xl-items="5" data-lg-items="4"  data-md-items="3" data-sm-items="2" data-xs-items="2" data-autoplay='true' data-infinute="true" data-dots="true">
                            @foreach ($shop->user->products->where(['admin_approval', 1] ,['published', 1] )->where('seller_featured', 1) as $key => $product)
                                <div class="carousel-box">
    ...
    <section class="mb-4">
        <div class="container">
            ...
            <div class="row gutters-5 row-cols-xxl-5 row-cols-lg-4 row-cols-md-3 row-cols-2">
                @php
                    if (!isset($type)){
                        $products = \App\Product::where('user_id', $shop->user->id)->where([['admin_approval', 1] ,['published', 1] ])->orderBy('created_at', 'desc')->paginate(24);
                    }
                    elseif ($type == 'top_selling'){
                        $products = \App\Product::where('user_id', $shop->user->id)->where([['admin_approval', 1] ,['published', 1] ])->orderBy('num_of_sale', 'desc')->paginate(24);
                    }
                    elseif ($type == 'all_products'){
                        $products = \App\Product::where('user_id', $shop->user->id)->where([['admin_approval', 1] ,['published', 1] ])->paginate(24);
                    }
                @endphp
