                ...
                <div class="aiz-user-panel">
                    ...
                    <div class="card">
                        <div class="modal-header">
                            ...
                            <h5 class="modal-title strong-600 heading-5">{{ translate('Order date')}}: {{ date('d-m-Y H:i A', $order->date) }}</h5>
                        ...
                        <div class="modal-body gry-bg px-3 pt-0">
                            ...
                            <div class="row mt-5">
                                ...
                                <div class="offset-lg-2 col-lg-4 col-sm-6">
                                    <div class="form-group">
                                        <input class="form-control"  placeholder="{{$payment_status}}" disabled>
                                ...
                                <div class="col-lg-4 col-sm-6">
                                    <div class="form-group">
                                        <input class="form-control"  placeholder="{{$status}}" disabled>
                            ...
                            <div class="row">
                                                ...
                                                <tbody>
                                                    @foreach ($order->orderDetails->where('seller_id', Auth::user()->id) as $key => $orderDetail)
                                                        <tr>
                                                            ...
                                                            @endif  --}}
                                                            <td>
                                                                @if ($orderDetail->item_status == 'pending' || $orderDetail->item_status == null)
                                                                <label> {{  translate('Item Availability') }} </label> <br> 
                                                                <input class="available_yes" data-id="{{ $orderDetail->id }}" type="radio"  name="item_status" value="available">
                                                                <label >{{  translate('Yes') }}</label>
                                                                <input class="available_no" data-id="{{ $orderDetail->id }}" type="radio"  name="item_status" value="not_available">
                                ...
                                <div class="container">
                                    ...
                                    $seller = Auth::user()->id;
                                @endphp
                                <a href="{{ route('seller.invoice.download', $order->id .'/'. $seller) }}" class="btn btn-soft-primary btn-icon btn-circle btn-sm" title="{{ translate('Download Invoice') }}">
...
<script type="text/javascript">
    ...
    $('.available_yes').on('click', function(){
        confirm("{{ translate('Are You Sure') }}");
        var order_id = {{ $order->id }};
        var status = $('.available_yes').val();
        var id = $(this).attr("data-id");
         //console.log(status);
        $.post('{{ route('orders.available_yes') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,status:status,id:id}, function(data){
            $('#order_details').modal('hide');
            //console.log(data);
            AIZ.plugins.notify('success', '{{ translate('Available status has been updated') }}');
            location.reload().setTimeOut(500);
