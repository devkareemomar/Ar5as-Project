...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $tax = 0;
        // dd($request->all());
       $conView =  !empty($request->session()->get('cart')) && !in_array($request->id,$request->session()->get('cart')->pluck('id')->toArray());
        ...
        if($request->session()->has('cart')){
            ...
            if (!$foundInCart && !empty($request->session()->get('cart')) && !in_array($request->id,$request->session()->get('cart')->pluck('id')->toArray())) {
                $cart->push($data);
            }
        ...
        if ($conView) {
            ...
            return array('status' => 1,'count' => count($request->session()->get('cart')->pluck('id')->toArray()), 'view' => view('frontend.ar5as.partials.outOfStockCart', compact('product', 'data'))->render());
        }
    }
    ...
    public function removeFromCart(Request $request)
    {
        ...
        return  view('frontend.ar5as.partials.outOfCart');
    }

    public function removeFromCartView(Request $request)
    {
        if($request->session()->has('cart')){
            $cart = $request->session()->get('cart', collect([]));
