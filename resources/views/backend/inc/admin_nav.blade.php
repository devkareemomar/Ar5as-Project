                    ...
                    <a class="dropdown-toggle no-arrow" data-toggle="dropdown" href="javascript:void(0);" role="button" aria-haspopup="false" aria-expanded="false">
                        <span class="btn btn-icon p-1">
                            <span class=" position-relative d-inline-block">
                               ...
                               <span class="badge  badge-danger position-absolute absolute-top-right">{{ $alertQty1->count() +  $alertQty2->count()}}  </span>
                            </span>
                        </span>
                    </a>
                     <div class="dropdown-menu dropdown-menu-right dropdown-menu-animated dropdown-menu-lg py-0">
                        <div class="p-3 bg-light border-bottom">
                            <h6 class="mb-0">{{ translate('Alert quantities') }} <a href="{{ route('products.alertQty') }}" class="btn btn-info"> {{ translate('Edit') }} </a> </h6>
                        </div>
                        <ul class="list-group c-scrollbar-light overflow-auto" style="max-height:300px;">

                           @foreach ($alertQty2 as $qty2)
                            <li class="list-group-item">
                                <a href="{{ route('products.alertQty') }}" class="text-reset">
                                    <span class="ml-2"> {{$qty2->product->getTranslation('name')}} 
                                         <span class="badge  badge-danger position-absolute absolute-top-right p-2"> {{ $qty2->qty }}  </span>
                            ...
                            <li class="list-group-item">
                                <a href="{{ route('products.alertQty') }}" class="text-reset">
                                    <span class="ml-2"> {{$qty1->getTranslation('name')}} 
                                         <span class="badge  badge-danger position-absolute absolute-top-right p-2"> {{ $qty1->current_stock }}  </span>
