            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td>
                            @php
                                $parentsub = \App\Category::where('id', $category->parent_id)->first();

                                $parent = \App\Category::where('id', $parentsub->parent_id)->first();
                            @endphp
                            @if ($parent != null)
                        ...
                        <td>
                            @php
                                $sub = \App\Category::where('id', $category->parent_id)->first();
                            @endphp
                            @if ($sub != null)
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_refund(this)" value="{{ $category->id }}" <?php if($category->refundable == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_refund(el){
            ...
            $.post('{{ route('categories.refund') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Refund categories updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
         ...
         function update_published(el){
            ...
            $.post('{{ route('categories.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Published categories updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
