            ...
            <tbody>
                @foreach($products as $key => $product)
                    <tr>
                        ...
                        <td>
							<label class="aiz-switch aiz-switch-success mb-0">
	                            <input onchange="update_admin_approval(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->admin_approval == 1) echo "checked";?> >
	                            <span class="slider round"></span>
							</label>
						</td>
                        @endif
...
@section('script')
    <script type="text/javascript">
        ...
        function update_admin_approval(el){
             if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('products.admin_approval') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Products updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
