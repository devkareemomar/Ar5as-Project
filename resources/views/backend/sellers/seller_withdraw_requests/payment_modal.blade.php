      ...
      <table class="table table-striped table-bordered" >
          <tbody>
                <tr>
                    @if($seller->available_balance >= 0)
                        ...
                        <td>{{ single_price($seller->available_balance) }}</td>
                    @endif
                </tr>
                <tr>
                    @if($seller_withdraw_request->amount > $seller->available_balance)
        ...
        @if ($seller->available_balance > 0)
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    @if ($seller_withdraw_request->amount > $seller->available_balance)
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ $seller->available_balance }}" class="form-control" required>
                    @else
            ...
            @else
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ abs($seller->available_balance) }}" class="form-control" required>
                    </div>
                </div>
                <div class="form-group row" id="txn_div">
                    <label class="col-md-3 col-from-label" for="txn_code">{{translate('Txn Code')}}</label>
