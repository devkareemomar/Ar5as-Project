<div class="aiz-user-sidenav-wrap pt-4 position-relative z-1 shadow-sm">
    ...
    <div class="aiz-user-sidenav rounded overflow-hidden  c-scrollbar-light">
        <div class="px-4 text-center mb-4">
            ...
            @else 
                    @php 
                         $shopName = App\Shop::where('user_id', Auth::user()->id)->select('name')->first();
                    @endphp 
                <h4 class="h5 fw-600">{{  $shopName->name }}
                    <span class="ml-2">
                ...
                <h6 class="h6 fw-600"> 
                   @php 
                         $seller_type = App\Seller::where('user_id', Auth::user()->id)->select('seller_type')->first();
                   @endphp 
                   @if( $seller_type->seller_type == 1)

                    {{ translate('Individual') }}
                   @elseif ($seller_type->seller_type == 2)
                    {{ translate('Company') }}
                   @endif
        ...
        <div class="sidemnenu mb-3">
            <ul class="aiz-side-nav-list" data-toggle="aiz-side-menu">
                ...
                @if(Auth::user()->user_type == 'seller')
                    ...
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('withdraw_requests.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['withdraw_requests.index'])}}">
                            ...
                            <span class="aiz-side-nav-text">{{ translate('سحب الرصيد') }}</span>
                        </a>
                    </li>
                ...
                <hr>
                <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.account.statment') }}" class="aiz-side-nav-link {{ areActiveRoutes(['seller.account.statment'])}}">
                        <i class="las la-money-bill aiz-side-nav-icon"></i>
                        <span class="aiz-side-nav-text">{{translate('Account statement')}}</span>
                    </a>
                </li>
                @endif
                ...
                <hr>
                <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.alertqty') }}" class="aiz-side-nav-link {{ areActiveRoutes(['seller.alertqty'])}}">
                        <i class="las la-money-bill aiz-side-nav-icon"></i>
                        <span class="aiz-side-nav-text">{{translate('Product Alert Quantaty')}}</span>
                    </a>
                </li>
                @endif
            </ul>
