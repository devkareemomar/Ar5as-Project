                    ...
                    <form class="" action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
                                ...
                                <div class="form-group row" >
                                    ...
                                    <div class="col-md-8">
                                        <select id="subsubcategory" class="form-control aiz-selectpicker" name="subsubcategory_id"  data-live-search="true" >
                                              <option>{{ translate('Select Sub Sub Category') }}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                @php
                                    $discount_from = date('d-m-Y H:i:s', $product->discount_from);
                                    $discount_to = date('d-m-Y H:i:s', $product->discount_to);
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-3">
                                        {{-- value="{{ $discount_from.' to '.$discount_to }}" --}}
                                        <input type="text" class="form-control aiz-date-range" name="date_range" value="" placeholder="{{translate('Select Date')}}" data-time-picker="true" data-format="DD-MM-Y HH:mm:ss" data-separator=" to " autocomplete="off">
                                    </div>
                                    <div class="col-lg-3">
                                ...
                                <div class="form-group row" id="alert_qty">
                                    ...
                                    <div class="col-md-6">
                                        <input type="number" lang="en" min="0" value="{{ $product->alert_qty }}" value="{{ $product->alert_qty }}" step="1" placeholder="{{ translate('Alert Quantity') }}" name="alert_qty" class="form-control" required>
                                    </div>
                                </div>
                                <br>
...
<script type="text/javascript">
        ...
        $(document).ready(function() {
            $.ajax({
                        url: "{{ route('subcatt') }}?cat_id=" + "{{ $product->category_id }}"+'&product_id=' + $('#product_id').val(),
                        method: 'GET',
                    ...
                    $.ajax({
                        url: "{{ route('subcatt') }}?cat_id=" + "{{ $product->subcategory_id }}"+'&product_id=' + $('#product_id').val(),
                        method: 'GET',
                        success: function(data) {
       ...
       $("#category").change(function(){
            $.ajax({
                url: "{{ route('subcatt') }}?cat_id=" + $(this).val()+'&product_id=' + $('#product_id').val(),
                method: 'GET',
                success: function(data) {
                    $('#subcategory').html(data.html);
                }
